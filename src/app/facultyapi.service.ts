...
export class FacultyapiService {
  ...
  updateFaculty(faculty): Observable<any> {
    const body = {facultyID: faculty.facultyID, fac_title: faculty.fac_title, fac_firstName: faculty.fac_firstName,
    fac_lastName: faculty.fac_lastName, fac_shortName: faculty.fac_shortName, fac_gender: faculty.fac_gender,
    fac_designation: faculty.fac_designation };
    return this.http.put(this.baseurl + '/faculties/' + faculty.facultyID + '/', body,
    {headers: this.httpHeaders});
  }
